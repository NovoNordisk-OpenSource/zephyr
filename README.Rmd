...
When using zephyr options the verbosity level is determined by checking the following sources in order:
 
- Global package-specific option: `options("[package].verbosity_level")`

- Package-specific environment variable: `Sys.getenv("R_[PACKAGE]_VERBOSITY_LEVEL")`
...
- Package-specific option set with `define_option_pkg("verbosity_level")`
