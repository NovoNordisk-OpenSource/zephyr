...
#' @param verbosity_levels `character` vector of levels of verbosity for which
...
#'           verbosity_levels = c("verbose", "debug"),
#'           msg_fun = cli::cli_h1)
#'
...
write_msg <- function(message,
  ...
  match.arg(verbosity_levels,
            choices = c("quiet", "verbose", "debug"),
            several.ok = TRUE)

  if (options::opt("verbosity_level") %in% verbosity_levels) {
    msg_fun(message, ...)
  }
